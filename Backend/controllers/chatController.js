...
const sendMessage = async (req, res) => {
    try {
        ...
        const createdMessage = await createMessage({ chat: chatId, user,  type: message.type, data:message.message });
        return successResponse(res, MESSAGE.SUCCESS_MESSAGE_SENT, createdMessage);
    } catch (err) {
        console.error(err);
        return errorResponse(res, MESSAGE.INTERNAL_SERVER_ERROR);
    }
